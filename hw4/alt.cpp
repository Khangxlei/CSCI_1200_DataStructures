  ...
  while (istr >> token) {
    if (token == "dvd") {
      ...
      for (int i = 0; i < num_movies; i++) {
        std::string dvd_name = read_dvd_name(istr);
        //Checks to see if the given movie preferences are in the inventory
...
;        for (inventory_type::iterator itr = inventory.begin(); itr!= inventory.end(); ++itr){
      ...
      shipping_algorithm(inventory,customers, ostr);
        ...
        //Error checking if customer not in file or has no DVDs
      ...
      //Error checking
      //Checks to see if the given movie is in the inventory
      ...
      //Error checking
      //Checks if given customer is in the list
      unsigned int customer_check=0;
